...
interface Student {
  ...
  grade: number | null; // stored directly on the enrollments row
}
...
const CourseDetails: React.FC = () => {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      // 2. Fetch Students (grade comes directly from the enrollments row)
      ...
      snapshotData?.forEach((s) => {
        if (!statsMap[s.user_id])
          statsMap[s.user_id] = { count: 0, latest: null };
        statsMap[s.user_id].count++;
        if (
          !statsMap[s.user_id].latest ||
          s.saved_at > statsMap[s.user_id].latest!
        ) {
      ...
      const formattedStudents = (enrollmentData || []).map((e: any) => ({
        ...
        grade: e.grade !== undefined ? e.grade : null,
      }));
  ...
  const getGradeBadgeStyle = (score: number): React.CSSProperties => {
    if (score >= 80) return { background: "#dcfce7", color: "#16a34a", fontWeight: 700 };
    if (score >= 60) return { background: "#fef9c3", color: "#a16207", fontWeight: 700 };
    return { background: "#fee2e2", color: "#dc2626", fontWeight: 700 };
  };
          ...
          <div>
            ...
            <p style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              ...
              <span style={{ fontWeight: 600, color: "var(--primary)" }}>
                Join Code: {courseInfo.join_code}
              </span>
            </p>
          </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <p className="text-muted" style={{ fontSize: "0.875rem" }}>
            Full record of students currently enrolled in this course.
          </p>
        </div>
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.id}>
                    ...
                    <td>
                      <span
                        className={`badge ${student.snapshot_count > 0 ? "badge-success" : "badge-warning"}`}
                      >
                    ...
                    <td>
                      {student.grade !== null ? (
                        <span
                          style={{
                            ...getGradeBadgeStyle(student.grade),
                        ...
                        >
                          <span>{getLetterGrade(student.grade)}</span>
                          <span style={{ opacity: 0.75 }}>({student.grade}%)</span>
                        </span>
                      ) : (
